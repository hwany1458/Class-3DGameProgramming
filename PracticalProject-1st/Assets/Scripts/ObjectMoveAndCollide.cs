...
public class ObjectMoveAndCollide : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.tag == "Item")
        {
            ...
            // 객체.SendMessage()를 쓰지 않고
            // (public으로 접근권한을 변경하여) GetComponent<>()로 변경
            //collision.gameObject.SendMessage("DestroySelf", transform.position);
            collision.gameObject.GetComponent<ObjectDestroyWithParticle>().DestroySelf(transform.position);
        }

        // 충돌 오브젝트가 벽이면
        if(collision.gameObject.tag == "Wall")
        {
            // 아이템 획득 점수(score)를 깎아준다
            //GameManager.MinusResource();
        }
    }
}
